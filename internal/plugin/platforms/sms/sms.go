...
func New() *Adapter {
	...
	return &Adapter{
		...
		accountSID: strings.TrimSpace(os.Getenv("SMS_ACCOUNT_SID")),
		authToken:  strings.TrimSpace(os.Getenv("SMS_AUTH_TOKEN")),
		fromNumber: strings.TrimSpace(os.Getenv("SMS_FROM_NUMBER")),
...
func (a *Adapter) Validate() error {
	if a.accountSID == "" || a.authToken == "" || a.fromNumber == "" {
		return fmt.Errorf("SMS_ACCOUNT_SID, SMS_AUTH_TOKEN and SMS_FROM_NUMBER must be set in environment")
	}
	return nil
}
...
func (a *Adapter) Start(ctx context.Context) error {
	...
	if a.running {
		a.mu.Unlock()
		return fmt.Errorf("sms: already running")
	}
	a.running = true
	ctx, a.cancel = context.WithCancel(ctx)
	a.mu.Unlock()

	addr := strings.TrimSpace(os.Getenv("SMS_WEBHOOK_LISTEN_ADDR"))
	if addr == "" {
		addr = ":8093"
	}

	a.whServer = plugin.NewWebhookServer(addr, a.logger)
	a.whServer.RegisterRoute("/sms/webhook", a.handleWebhook)
	...
	if err := a.whServer.Start(ctx); err != nil {
		return fmt.Errorf("sms: server start failed: %w", err)
	}

	a.logger.Info("sms: listening", "addr", addr)
	return nil
}
...
func (a *Adapter) SendMessage(ctx context.Context, channelID string, msg plugin.OutgoingMessage) error {
	a.logger.Info("sms: send", "to", channelID, "text_len", len(msg.Text))
	return nil
}

func (a *Adapter) handleWebhook(ctx context.Context, body []byte, headers http.Header) (*plugin.IncomingMessage, error) {
	a.logger.Info("sms: received webhook", "body_len", len(body))
	return nil, nil
}
