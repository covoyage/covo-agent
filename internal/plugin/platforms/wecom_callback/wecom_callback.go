...
const PlatformName = "wecom_callback"
...
func New() *Adapter {
	...
	return &Adapter{
		corpID:         strings.TrimSpace(os.Getenv("WECOM_CALLBACK_CORP_ID")),
		token:          strings.TrimSpace(os.Getenv("WECOM_CALLBACK_TOKEN")),
		encodingAESKey: strings.TrimSpace(os.Getenv("WECOM_CALLBACK_ENCODING_AES_KEY")),
		agentID:        strings.TrimSpace(os.Getenv("WECOM_CALLBACK_AGENT_ID")),
...
func (a *Adapter) Validate() error {
	if a.corpID == "" || a.token == "" || a.encodingAESKey == "" {
		return fmt.Errorf("WECOM_CALLBACK_CORP_ID, WECOM_CALLBACK_TOKEN and WECOM_CALLBACK_ENCODING_AES_KEY must be set in environment")
	}
	return nil
}
...
func (a *Adapter) Start(ctx context.Context) error {
	if err := a.Validate(); err != nil {
		return fmt.Errorf("wecom_callback: validation failed: %w", err)
	}

	a.mu.Lock()
	if a.running {
		a.mu.Unlock()
		return fmt.Errorf("wecom_callback: already running")
	}
	a.running = true
	ctx, a.cancel = context.WithCancel(ctx)
	a.mu.Unlock()

	addr := strings.TrimSpace(os.Getenv("WECOM_CALLBACK_LISTEN_ADDR"))
	if addr == "" {
		addr = ":8089"
	}

	a.whServer = plugin.NewWebhookServer(addr, a.logger)
	a.whServer.RegisterRoute("/wecom/callback", a.handleWebhook)
	...
	if err := a.whServer.Start(ctx); err != nil {
		return fmt.Errorf("wecom_callback: server start failed: %w", err)
	}

	a.logger.Info("wecom_callback: listening", "addr", addr)
	return nil
}
...
func (a *Adapter) Stop() error {
	...
	a.running = false
	a.logger.Info("wecom_callback: stopped")
	return nil
}
...
func (a *Adapter) SendMessage(ctx context.Context, channelID string, msg plugin.OutgoingMessage) error {
	a.logger.Info("wecom_callback: send", "channel", channelID, "text_len", len(msg.Text))
	return nil
}

func (a *Adapter) handleWebhook(ctx context.Context, body []byte, headers http.Header) (*plugin.IncomingMessage, error) {
	a.logger.Info("wecom_callback: received webhook", "body_len", len(body))
	return nil, nil
}
