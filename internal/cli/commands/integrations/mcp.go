...
func NewMCPCommand() *cobra.Command {
	...
	testCmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, args []string) error {
			cmdMCP(shared.PrependArg("test", args))
			return nil
		},
	}
	...
	addCmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, args []string) error {
			cmdMCP(shared.PrependArg("add", args))
			return nil
		},
	}
	...
	mcpCmd.AddCommand(&cobra.Command{Use: "remove <name>", Short: "Remove MCP server config", Args: cobra.ExactArgs(1), RunE: func(_ *cobra.Command, args []string) error { cmdMCP(shared.PrependArg("remove", args)); return nil }})
...
func cmdMCPServe() {
	...
	agentCfg := agent.CovoAgentConfig{
		...
		HomeDir:                  homeDir,
		Logger:                   logger,
		MCPServers:               shared.MCPAgentConfig(cfg),
