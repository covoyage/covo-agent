...
import (
	...
	cmdsession "github.com/covoyage/covo-agent/internal/cli/commands/session"
	...
	return &slashcmd.ContextBuilder{
		...
		IO: slashcmd.IODependencies{
			...
			ImportSessionFromJSONL: func(ctx context.Context, manager interface{}, path string) (string, int, error) {
				return cmdsession.ImportSessionFromJSONL(ctx, manager.(*session.Manager), path)
			},
		},
		Services: slashcmd.ServiceDependencies{
			...
			WriteSkinTheme:      prefs.WriteSkinTheme,
			NotifyGatewayFooter: integrations.NotifyGatewayFooter,
