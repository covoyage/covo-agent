...
func promptProvider(currentProvider string) string {
	providers := model.ProviderChoices()

	if !cli.IsTerminal(os.Stdin.Fd()) {
		return shared.SelectOne("Select AI provider", providers, currentProvider)
	}
	...
	if err != nil {
		return shared.SelectOne("Provider", providers, currentProvider)
	}
	...
	for {
		...
		if n == 1 {
			switch buf[0] {
			case 13, 10: // Enter
				shared.ClearSelect(len(items) + 1)
...
func promptMode(currentMode string) string {
	...
	return shared.SelectOne("Default mode", modes, currentMode)
}
