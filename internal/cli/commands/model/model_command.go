...
func selectProviderInteractive(label string, options []string, defaultVal string) (string, int) {
	...
	if !cli.IsTerminal(os.Stdin.Fd()) {
		choice := shared.PromptChoice(bufio.NewReader(os.Stdin), label, options, defaultVal)
		return choice, 0
	}
	...
	shared.RenderProviderList(label, filtered, selected, offset, pageSize, search, searching, defaultVal)
...
func editCustomProvider(cfg *cli.Config, reader *bufio.Reader, providerType string) {
	...
	protocol := shared.SelectOneOrCancel("Protocol type", protocolOptions, target.Protocol)
	if protocol == "" {
		return
	}
...
func addCustomProvider(_ *bufio.Reader, cfg *cli.Config) (*cli.CustomProvider, string, error) {
	...
	// 2. Protocol type
	protocol := shared.SelectOneOrCancel("Protocol type", protocolOptions, "")
	if protocol == "" {
		return nil, "", nil
	}
...
func selectModelFromChoices(reader *bufio.Reader, options []string, currentModel string) string {
	choice := shared.SelectOne("Select default model", options, currentModel)
...
// NewModelCommand builds the `model` subcommand for provider and model selection.
func NewModelCommand(runtime *cli.CommandRuntime) *cobra.Command {
