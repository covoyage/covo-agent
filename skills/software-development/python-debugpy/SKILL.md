...
Note: pytest with xdist (`-n 4`) by default, and pdb does NOT work under xdist. Add `-p no:xdist` or run a single test with `-n 0`:
...
For long-lived processes: gateway, daemon, a process that's already misbehaving and can't be restarted clean.
...
8. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For subagents, debug one process at a time.
