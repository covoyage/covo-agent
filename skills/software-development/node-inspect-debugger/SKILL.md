...
Install to a throwaway location if you don't want to dirty the project:
...
6. **Running `node inspect` through an agent terminal.** It's a PTY-friendly REPL. Launch it with `bash` in interactive mode. Non-interactive foreground mode will work for one-shot commands but not for interactive stepping.
