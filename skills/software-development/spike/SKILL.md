...
For libraries without docs pages, clone and read their `README.md` / `examples/` via `read`.
...
**Parallel comparison spikes (002a / 002b) — delegate.** When two approaches can run in parallel and both need real engineering (not 10-line prototypes), fan out with `sessions_spawn`:

```
sessions_spawn(
    task="Build 002a-pdf-parse-pdfjs: ...",
    toolsets=["shell", "filesystem", "web"]
)
sessions_spawn(
    task="Build 002b-pdf-parse-camelot: ...",
    toolsets=["shell", "filesystem", "web"]
)
```
