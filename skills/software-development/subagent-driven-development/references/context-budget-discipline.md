...
1. **Never read agent definition files.** `sessions_spawn` auto-loads them — you reading them too just doubles the cost.
2. **Never inline large files into subagent prompts.** Tell the agent to read the file from disk with `read` instead. The subagent gets full content; your context stays lean.
...
Check the model's actual context window. If you don't know, assume the smaller one — err toward leanness.
