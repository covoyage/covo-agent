...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.covo-agent/.env ] && grep -q "^GITHUB_TOKEN=" ~/.covo-agent/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.covo-agent/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Covo Agent\",
    \"comments\": [
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Covo Agent. Code looks clean — good test coverage, no security concerns."
...
In addition to inline comments, leave a top-level summary so the PR author gets the full picture at a glance.
