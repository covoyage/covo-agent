#!/usr/bin/env bash
# GitHub environment detection helper for Covo Agent skills.
#
# Usage (via shell tool):
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif [ -f "$HOME/.covo-agent/.env" ] && grep -q "^GITHUB_TOKEN=" "$HOME/.covo-agent/.env" 2>/dev/null; then
    GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$HOME/.covo-agent/.env" | head -1 | cut -d= -f2 | tr -d '\n\r')
