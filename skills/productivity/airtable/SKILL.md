...
Work with Airtable's REST API directly via `curl` using the `shell` tool. No MCP server, no OAuth flow, no Python SDK — just `curl` and a personal access token.
...
4. Store the token in `~/.covo-agent/.env` (or via `covo setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for Covo Agent. Pipe through `python3 -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
## Important Notes for Covo Agent

- **Always use the `shell` tool with `curl`.** Do NOT use `web_extract` (it can't send auth headers) or `browser_navigate` (needs UI auth and is slow).
- **`AIRTABLE_API_KEY` flows from `~/.covo-agent/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
