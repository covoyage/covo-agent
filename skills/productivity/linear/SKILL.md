...
2. Set `LINEAR_API_KEY` in your environment (via `covo setup` or your env config)
...
```bash
SCRIPT=$(dirname "$(find ~/.covo-agent -path '*skills/productivity/linear/scripts/linear_api.py' 2>/dev/null | head -1)")/linear_api.py
...
- Always use `shell` tool with `curl` for API calls — do NOT use `web_extract` or `browser`
